...
A [Windhawk](https://windhawk.net) mod for the Windows 11 taskbar: an app you
close and open again goes back to its place, instead of to the end.
...
button is at the end. The same happens to every app after a restart: they come
back in whatever order they happen to start. This mod remembers where each app
was and puts its button back there.
...
taskbar now. When none of those are there, it goes in front of the apps on the
taskbar that the list has.
...
Only what happens while the mod is running is learned. Buttons rearranged while
it's disabled go back to their old places when their apps reopen; drag them
...
- a new app's group goes in at the index the remembered order gives, instead of
  the end. That index is the one the taskbar's XAML view is then told about, so
  the button simply appears in its place, with no jump;
- after a drag, the order learns where the app now is.

No offsets inside Windows' objects are used: the list is the one passed to
`DPA_InsertPtr`, and everything else comes from Microsoft's public symbols,
which Windhawk downloads.
...
- **With several monitors** there is one list for all the taskbars, and each
  ...
  other side. With the two "taskbar where window is open" settings, an app
...
- **With the [Taskbar Grouping](https://windhawk.net/mods/taskbar-grouping)
  mod**, which can give each window a button of its own, an app's first button
  goes back to its place, and the buttons of its other windows go where Taskbar
  Grouping's settings say. Their places aren't remembered: an app that sat right
  ...
  kept. When the first window closes, Taskbar Grouping passes its identity on to
  another of the app's windows, and that window's button becomes the app's
  place.
...
| Path | What |
| --- | --- |
| `%ProgramData%\Windhawk\Engine\ModsWritable\mod-storage\<mod folder>\<your SID>\order.txt` | the remembered order, one app per line |
...
Its first line is `taskbar-remember-positions v1`, and each line after it is one
app, in order: the day it was last seen, a tab, and its App ID. A file the mod
can't read is moved aside as `order.txt.<date>-<time>.bad`; one it can only read
in part is copied there, then rewritten with the lines it could read.
Uninstalling the mod deletes the folder. Past 256 apps, the ones not seen for
the longest time are forgotten.
